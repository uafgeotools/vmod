...
class Yang(Source):
    ...
    def stress(self, x, y, z, xcen, ycen, z0, P, a, b, phi, theta ,mu=9.6e9,nu=0.25):
        ...
        u,v,w=self.model_depth(x, y, z, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        
        upx,vpx,wpx=self.model_depth(x+h, y, z, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        umx,vmx,wmx=self.model_depth(x-h, y, z, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        dudx=0.5*(upx-umx)/h
        dvdx=0.5*(vpx-vmx)/h
        dwdx=0.5*(wpx-wmx)/h
        
        upy,vpy,wpy=self.model_depth(x, y+h, z, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        umy,vmy,wmy=self.model_depth(x, y-h, z, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        dudy=0.5*(upy-umy)/h
        dvdy=0.5*(vpy-vmy)/h
        dwdy=0.5*(wpy-wmy)/h
        
        if z==0:
            upz,vpz,wpz=self.model_depth(x, y, z+2*h, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
            umz,vmz,wmz=self.model_depth(x, y, z, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        else:
            upz,vpz,wpz=self.model_depth(x, y, z+h, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
            umz,vmz,wmz=self.model_depth(x, y, z-h, xcen, ycen, z0, P, a, b, phi, theta, mu=mu,nu=nu)
        dudz=0.5*(upz-umz)/h
        dvdz=0.5*(vpz-vmz)/h
        dwdz=0.5*(wpz-wmz)/h
...
def verify(A=0.5,theta=30,phi=60):
    ...
    ux,uy,uz=yan.model(x,y,xcen=xcen,ycen=ycen,z0=z0,P=dP,a=a,b=b,phi=phi,theta=theta,mu=mu,nu=0.25)
...
def verify1(A=0.5,theta=30,phi=60):
    ...
    #print(xcen,ycen,z0,dP,a,b,phi,theta,mu,0.25)
    inicio=time.time()
    ux,uy,uz=yan.model(x,y,xcen=xcen,ycen=ycen,z0=z0,P=dP,a=a,b=b,phi=phi,theta=theta,mu=mu,nu=25)
    fin=time.time()
    ...
    ax2.scatter(x/1000,uy,c='blue')
    ax2.set_ylabel('V north (meters)')
    ax2.set_xlabel('X in km')
    #ax2.set_xlim([-10,10])
    #ax2.set_ylim([-0.6,0.8])

    ax3.scatter(x/1000,uz,c='red')
    ax3.set_ylabel('W up (meters)')
    #ax3.set_xlim([-10,10])
    #ax3.set_ylim([0,2.5])
